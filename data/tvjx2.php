...
mkdir('./data');
mkdir('./data/runtime');
$gxpd=time()-filemtime('./data/runtime/'.md5($tvurllist));
if($gxpd>2*60*60){
$tvinfo=file_get_contents($tvurllist);
file_put_contents('./data/runtime/'.md5($tvurllist),gzdeflate($tvinfo));
}	
$tvinfo=gzinflate(file_get_contents('./data/runtime/'.md5($tvurllist)));
