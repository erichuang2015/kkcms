...
mkdir('./data');
mkdir('./data/runtime');
$gxpd=time()-filemtime('./data/runtime/'.md5($zyurllist));
if($gxpd>2*60*60){
$zyinfo=file_get_contents($zyurllist);
file_put_contents('./data/runtime/'.md5($zyurllist),gzdeflate($zyinfo));
}	
$zyinfo=gzinflate(file_get_contents('./data/runtime/'.md5($zyurllist)));
